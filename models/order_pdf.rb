class OrderPdf
  ...
  def line_items
    ...
    table << [{ content: "Barzahlungsrabatt ab CHF #{@order.cash_discount_treshold}: #{@order.cash_discount_percentage}%", colspan: 2 }, '', formatted_price(@order.cash_discount), formatted_price(@order.cash_discounted_price)]
    table << [{ content: "Mengenrabatt ab CHF #{@order.bulk_discount_treshold}: #{@order.bulk_discount_percentage}%", colspan: 2 }, '', formatted_price(@order.bulk_discount), formatted_price(@order.bulk_discounted_price)]
    if @order.spring_discount?
      table << [{ content: "Frühlingsrabatt: #{@order.spring_discount_percentage}%", colspan: 2 }, '', formatted_price(@order.spring_discount), formatted_price(@order.spring_discounted_price)]
    end
    table << [{ content: "MWST #{@order.vat_percentage}%", colspan: 2 }, '', formatted_price(@order.vat_amount), '']
    if @order.shipping_cost?
      table << [{ content: 'Versandkosten', colspan: 2 }, '', formatted_price(@order.shipping_cost), formatted_price(@order.price_with_shipping_cost)]
    end
    table << [{ content: "Total inkl. #{@order.vat_percentage}% MWST", colspan: 2 }, '', '', "CHF #{@order.total_price_f}"]
    ...
    subtotal_row = ((@order.spring_discount? && @order.shipping_cost?) ? -7 : ((@order.spring_discount? || @order.shipping_cost?) ? -6 : -5))
    table.row(subtotal_row).style font_style: :bold, borders: [:top, :bottom, :left, :right]
